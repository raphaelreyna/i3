...
void x_push_changes(Con *con) {
    ...
    if (warp_to) {
        ...
        if (!pointerreply) {
            ...
            int target_x = warp_to->x + (config.targeted_warping.x_position * warp_to->width / 100);
            int target_y = warp_to->y + (config.targeted_warping.y_position * warp_to->height / 100);

            // Nudge away from borders
            if (config.targeted_warping.x_position <= 0) {
                target_x = warp_to->x + 1;
            } else if (config.targeted_warping.x_position >= 100) {
                target_x = warp_to->x + warp_to->width - 1;
            }
            if (config.targeted_warping.y_position <= 0) {
                target_y = warp_to->y + 1;
            } else if (config.targeted_warping.y_position >= 100) {
                target_y = warp_to->y + warp_to->height - 1;
            }

            Output *current = get_output_containing(pointerreply->root_x, pointerreply->root_y);
            Output *target = get_output_containing(target_x, target_y);
            if (current != target) {
                ...
                xcb_warp_pointer(conn, XCB_NONE, root, 0, 0, 0, 0, target_x, target_y);
